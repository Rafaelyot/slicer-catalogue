...
from exceptions.exceptions import MalFormedException
...
def create_vs_descriptor(data):
    vs_blueprint_id, tenant_id, nested_vsd_ids = data.get('vs_blueprint_id'), data.get('tenant_id'), data.get(
        'nested_vsd_ids')
    vs_blueprint = VsBlueprint.get_or_404(vs_blueprint_id=vs_blueprint_id)  # get_vs_blueprints??
